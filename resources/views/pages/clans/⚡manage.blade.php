...
/*
 * Manage a clan, 1:1 from ClanManage.dc.html. Captains see it; only the
 * founder (owner) signs player changes, because the clan event (kind 32150)
 * and the lineups the league publishes are signed with the founder's key.
...
new #[Layout('layouts::app', ['section' => 'clans'])] class extends Component {
    ...
    public string $mode = '3v3';

    public string $role = 'substitute';
    ...
    public function prepareInvite(ClanService $clans): ?array
    {
        ...
        return $this->attempt(fn () => $clans->prepareInvite($this->user(), $this->clan, 'rocket-league', $this->mode, $invitee, LineupRole::from($this->role)), 'player');
    }
    ...
    public function invite(string $signed, ClanService $clans): void
    {
        ...
        $invite = $this->attempt(fn () => $clans->invite($this->user(), $this->clan, 'rocket-league', $this->mode, $invitee, LineupRole::from($this->role), $this->signed($signed)), 'player');

        if ($invite !== false) {
            $this->inviteLink = $invite instanceof ClanInvite ? route('invites.show', $invite) : null;
            $this->player = '';
            unset($this->clan);
        }
    }
    ...
    {
        $this->validate([
            'mode' => ['required', Rule::in(array_keys(app(GameRegistry::class)->get('rocket-league')->modes()))],
            'role' => ['required', Rule::in(['player', 'substitute', 'captain'])],
            'player' => ['required', 'string', 'max:200'],
        ]);
...
<div class="mx-auto flex w-full max-w-[1200px] grow flex-col gap-5 px-4 pb-6 lg:px-0"
    ...
    <section id="invite" class="flex flex-col gap-3.5 rounded-lg bg-card px-4 py-5 lg:px-6">
        <span class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1"><h2 class="m-0 text-[15px] font-bold">{{ __('Invite a player') }}</h2><span class="text-xs text-ink-3">{{ __('You get an invite link to send them.') }}</span></span>
        @if ($isOwner)
            <div class="grid grid-cols-1 items-end gap-3.5 lg:grid-cols-[220px_200px_minmax(0,1fr)_auto]">
                <fieldset class="m-0 flex flex-col gap-2 border-0 p-0"><legend class="pb-2 text-xs text-ink-2">{{ __('Lineup') }}</legend>
                    <span class="grid h-11 grid-cols-3 overflow-hidden rounded-lg shadow-ring">
                        @foreach (['3v3', '2v2', '1v1'] as $option)
                            <label @class(['flex cursor-pointer items-center justify-center text-[13px]', 'bg-btc font-bold text-on-btc' => $mode === $option, 'text-ink-2' => $mode !== $option])>
                                <input type="radio" wire:model.live="mode" value="{{ $option }}" class="sr-only">{{ $option }}
                            </label>
                        @endforeach
                    </span>
                </fieldset>
                <label class="flex flex-col gap-2"><span class="text-xs text-ink-2">{{ __('Role') }}</span>
                    <select wire:model="role" class="h-11 w-full appearance-none rounded-lg border border-edge bg-ground px-3.5 text-[13px] text-ink">
                        <option value="substitute">{{ __('Sub') }}</option>
                        <option value="player">{{ __('Player') }}</option>
                        <option value="captain">{{ __('Captain (only yourself)') }}</option>
                    </select>
                </label>
                <label class="flex flex-col gap-2"><span class="text-xs text-ink-2">{{ __('Player') }}</span>
                    <input type="search" wire:model="player" placeholder="{{ __('Name or profile link') }}" class="h-11 w-full rounded-lg border border-edge bg-ground px-3.5 text-[13px] text-ink placeholder:text-ink-3">
                </label>
                <button type="button" x-on:click="run('prepareInvite', 'invite')" x-bind:disabled="busy"
            ...
            @if ($inviteLink)
                <p class="m-0 flex flex-wrap items-center gap-2 text-xs text-win" role="status">
                    ...
                    <span class="min-w-0 truncate text-ink">{{ $inviteLink }}</span>
            ...
            @endif
            <span class="flex items-center gap-2 text-xs text-ink-2"><x-icon name="alert" :size="14" class="text-btc-hi" />{{ __('One player, one clan: accepting also leaves their current clan.') }}</span>
        @else
    ...
    <section class="flex flex-col gap-2.5 rounded-lg bg-card px-4 py-5 lg:px-6">
        ...
        @if ($tab === 'active')
            <div class="hidden grid-cols-[110px_minmax(0,1fr)_190px_100px] gap-3 px-2 pt-1 text-xs text-ink-3 lg:grid"><span>{{ __('Lineup') }}</span><span>{{ __('Seats') }}</span><span>{{ __('Status') }}</span><span>Elo</span></div>
            @forelse ($lineups as $lineup)
                @php($stats = ClanStatsPreview::lineup($tag, $lineup->mode))
                @php($needed = $lineup->gameMode()->lineupMinimum())
                <div wire:key="lu-{{ $lineup->id }}" class="tr grid min-h-[60px] grid-cols-[104px_minmax(0,1fr)] items-center gap-3 rounded-sm border-b border-hairline p-2 lg:grid-cols-[110px_minmax(0,1fr)_190px_100px]">
                    <span class="flex flex-col gap-0.5"><b class="text-[15px]">{{ $lineup->mode }}</b><x-rank-badge :tier="$stats['tier']" :level="$stats['level']" class="font-normal" /></span>
                    <span class="flex flex-wrap gap-2">
                        @foreach ($lineup->seats->sortBy('id') as $seat)
                            @php($active = $seat->isActive($clan->id))
                            <span @class(['flex h-[34px] items-center gap-1.5 rounded-md px-2.5 text-xs', 'border border-line bg-well' => $active, 'border border-dashed border-edge text-ink-2' => ! $active])>
                                {{ $seat->user->displayName() }}
                                @if ($seat->role !== LineupRole::Player || ! $active)<span class="text-ink-3">{{ $seat->role->label() }}@if (! $active), {{ __('invited') }}@endif</span>@endif
                            </span>
                        @endforeach
                        @for ($open = $lineup->listedCount(); $open < $needed; $open++)
                    ...
                    <span @class(['col-start-2 text-[13px] lg:col-start-auto', 'text-win' => $lineup->isReady(), 'text-btc-hi' => ! $lineup->isReady()])>
                        {{ $lineup->isReady() ? __('ready, :n of :m', ['n' => $lineup->activeCount(), 'm' => $needed]) : __('needs players, :n of :m', ['n' => $lineup->activeCount(), 'm' => $needed]) }}
                    </span>
                    <b class="col-start-2 font-display text-[15px] lg:col-start-auto">{{ $stats['elo'] }}</b>
                </div>
            @empty
                <p class="m-0 py-4 text-[13px] text-ink-2">{{ __('No lineups yet. Invite players to a 3v3, 2v2 or 1v1 above; take your own seat by entering your name.') }}</p>
            @endforelse
        ...
        @elseif ($tab === 'invited')
            @forelse ($pending as $invite)
                <div wire:key="iv-{{ $invite->id }}" class="grid min-h-[52px] grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-hairline px-2 text-[13px]">
                    <span class="flex min-w-0 flex-col gap-0.5"><b class="truncate">{{ $invite->invitee->displayName() }}</b><span class="text-xs text-ink-3">{{ __(':role in the :mode, sent :when', ['role' => $invite->role->label(), 'mode' => $invite->lineup->mode, 'when' => $invite->created_at?->diffForHumans()]) }}</span></span>
