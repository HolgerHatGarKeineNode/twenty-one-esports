...
--}}
@if (! $user->exists)
    {{-- A deleted account (ChessGame::deletedPlayer()): the name only, no page to open. --}}
    <span {{ $attributes->class('text-ink-2') }} data-test="deleted-player">{{ $slot->isEmpty() ? $user->displayName() : $slot }}</span>
@else
