 ...
 *   php tests/Support/mini-relay.php 7799 [events.json] ['{"max_filters":3,"advertised_max_filters":10,"ignore_limits":true}']
...
$limits = isset($argv[3]) ? json_decode((string) $argv[3], true) : [];

(new MiniRelay)->seed(is_array($seed) ? array_values($seed) : [])->limits(is_array($limits) ? $limits : [])->run((int) ($argv[1] ?? 7799));
