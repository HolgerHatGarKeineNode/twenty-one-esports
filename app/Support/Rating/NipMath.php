...
/**
 * The two numeric primitives the NIP fixes (docs/nips/esports.md, "Rating"
 * and "Global Rating"): rounding halves away from zero, and the inverse of the
 * standard normal distribution function by AS 241, as in Python's
 * `statistics.NormalDist().inv_cdf`, in IEEE 754 double precision.
 */
final class NipMath
{
    /**
     * Round to the nearest integer, halves away from zero. PHP's round() does
     * exactly that (PHP_ROUND_HALF_UP is "away from zero").
     */
    ...
    /**
     * Wichura, M.J. (1988), Algorithm AS241, the same coefficients and
     * evaluation order as CPython's `statistics._normal_dist_inv_cdf`.
     */
    public static function inverseNormal(float $p): float
    {
        if ($p <= 0.0 || $p >= 1.0) {
            throw new InvalidArgumentException('p must lie strictly between 0 and 1.');
        }
